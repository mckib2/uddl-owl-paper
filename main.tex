...
\begin{abstract}
The engineering of complex, software-intensive systems faces a pervasive challenge: data ambiguity.  As disparate systems ranging from avionics to industrial IoT (Internet of Things) attempt to exchange information, the semantic context of that data is often not communicated clearly and effectively, leading to additional unplanned integration effort and interoperability gaps.  The Universal Domain Description Language (UDDL) addresses this by providing a platform-independent data modeling language rooted in the separation of concerns between Conceptual, Logical, and Platform data models~\cite{OpenGroup2023UDDL}.  However, UDDL constructs are not well-studied in systems and interoperability literature while the Web Ontology Language (OWL) \cite{W3C_OWL2} and the Resource Description Framework (RDF) \cite{W3C_RDF11} (the de facto standard for semantic knowledge representation on the web) are actively researched and have rich tooling ecosystems.  We recognize UDDL use-cases as specializations of Sematic Web Technologies (SWTs) use-cases and that by bridging technology concepts both communities stand to gain insights in creating and utilizing data semantics. 
...
In systems engineering, rigorous definition of data and interfaces is not merely a documentation exercise.  It is a fundamental requirement for the composition, maintenance, and future reuse of complex systems.  In traditional systems engineering, data dictionaries and interface control documents (ICDs) have served as the primary means of semantic capture.  (TODO: cite some things here).  However, these artifacts are often static in nature and reliant on human interpretation.  As noted in the UDDL Technical Specification, ``Adequately describing domain data is a common problem in computer-based systems. Typical approaches involve creating documentation to accompany artifacts \ldots in an attempt to capture the semantics of the data exchanged''~\cite{OpenGroup2023UDDL}.

The failure of the traditional, document-based approaches to document data semantics stems from their inability to formally and precisely bind meaning to structure.  A variable labeled \texttt{alt} in a ICD may imply ``altitude'', but it may fail to specify a reference frame (e.g., ellipsoidal vs. geoidal), the unit of measure (meters vs. feet), or the conceptual domain (air operations vs. ground).  UDDL was specifically developed to fill this documentation gap, its objective being to ``formally document the meaning of data with the goal of eliminating ambiguity''~\cite{OpenGroup2023UDDL}.  It achieves this by enforcing a strict separation of concerns, distinguishing the identity of data from its context, and providing building blocks to construct formal definitions of the meaning of data flowing through interfaces.

Parallel to the development of UDDL in the systems engineering space, the Semantic Web has evolved a robust stack of technologies designed for machine-readability and logical inference.  At the core of this stack are RDF (Resource Description Framework) and OWL (Web Ontology Language).  RDF provides a graph-based data model where knowledge is atomized into triples (Subject, Predicate, Object), offering high flexibility and linkability~\cite{W3C_RDF11}.  OWL extends RDF with formal logical semantics (Description Logics), enabling systems to define complex relationships, cardinality constraints, and classifications~\cite{W3C_OWL2}.
...
\begin{table}[H]
    ...
    \begin{tabular}{|p{0.25\linewidth}|p{0.35\linewidth}|p{0.3\linewidth}|}
        ...
        \hline
        Conceptual Data Model (CDM) & Defines the complete semantics of all Entities and Associations. & Concepts and Traits: Entities and Associations are typed with Observables (traits which can be observed but not further characterized). \\
        \hline
...
Used for Meaning and Inference (resolving synonyms, inferring paths).

\subsubsection{SPARQL}

\subsubsection{SHACL}

is used for Structure Validation (enforcing the strict cardinality rules of UDDL)


\section{Theory}

In this section we present mappings between UDDL and SWTs along with supporting justifications.  In general, we will rely on RDF and OWL for core UDDL element and relationship mappings.  For UDDL Queries and Views we will leverage additional SWTs including SPARQL and SHACL.
...
% \subsection{The Observable Dilemma: Class vs. Datatype}
% The user posed a critical question regarding Observables: ``Maybe \texttt{rdfs:Datatype} works better? Not sure. Observables are `traits which can be observed, but not further characterized' and at the conceptual level have no units or types associated with them.''

% To answer this, we must look at the lifecycle of an Observable in UDDL.
% \begin{enumerate}
%     \item \textbf{Conceptual}: Observable (e.g., ``Temperature''). Abstract, no unit.
%     \item \textbf{Logical}: Measurement (e.g., ``CelsiusTemp''). Realizes the Observable. Has a Unit (``DegreeCelsius'') and Value Type (``Real'').
%     \item \textbf{Platform}: PlatformDataType (e.g., ``float'').
% \end{enumerate}

% \subsubsection{Analysis of rdfs:Datatype}
% If we map a Conceptual Observable (e.g., ``Temperature'') to \texttt{rdfs:Datatype} or \texttt{owl:DatatypeProperty}, we assert that it relates an Entity to a Literal (a raw value like ``25.0'').
% \begin{itemize}
%     \item \textbf{Pros}: Simple. Matches the idea that an observable is a ``trait.''
%     \item \textbf{Cons}: It creates a ``Semantic Dead End.'' In the Logical model, ``Temperature'' becomes a complex structure with a Unit and a Reference Frame. You cannot easily refine a Datatype Property (which points to a literal) into an Object Property (which points to a complex Measurement object) without breaking the inheritance hierarchy. In OWL, \texttt{DatatypeProperty} and \texttt{ObjectProperty} are disjoint.
% \end{itemize}

% \subsubsection{Analysis of owl:Class}
% If we map an Observable to an \texttt{owl:Class} (specifically, a subclass of \texttt{sosa:ObservableProperty} or \texttt{qudt:QuantityKind}):
% \begin{itemize}
%     \item \textbf{Pros}: It remains an object. We can define a property \texttt{hasObservable} that links an Entity to this Class. In the Logical model, we simply subclass the Observable Class to create a Measurement Class (e.g., \texttt{CelsiusTemp subClassOf Temperature}). This preserves the ``Realization'' relationship as a natural taxonomic or realization link.
%     \item \textbf{Cons}: Slightly more verbose.
% \end{itemize}

% \subsubsection{The Verdict: Observables are Classes}
% Given UDDL's requirement for refinement into Measurements~\cite{OpenGroup2023UDDL}, Observables must be mapped to \texttt{owl:Class}. They represent the concept of the quality, not the literal value of the quality. This aligns with standard ontologies like BFO (Basic Formal Ontology) which treats ``Quality'' as a Dependent Continuant (an entity), and SSN (Semantic Sensor Network) which treats properties as observable entities.

% \textbf{Mapping Rule:}
% \[ \text{UDDL:Observable}(O) \rightarrow \text{OWL:Class}(O) \]
...
% # The Entity having the trait
% ex:Engine a owl:Class ;
%     rdfs:subClassOf [
%         a owl:Restriction ;
%         owl:onProperty ex:hasTemperature ;
%         owl:someValuesFrom ex:Temperature
%     ].
% \end{verbatim}
...
% \section{Conclusion}
% The analysis confirms that the Universal Domain Description Language (UDDL) is highly compatible with Semantic Web Technologies, specifically OWL 2 DL. While there is no single ``magic button'' to convert MOF-based UDDL to DL-based OWL, a robust mapping exists by applying standard ontological patterns:
% \begin{itemize}
%     \item Entities and Basis Entities map naturally to OWL Classes.
%     \item Observables must be mapped to OWL Classes (Qualities) to allow for future refinement, avoiding the trap of rigid Datatype properties.
%     \item Associations require Reification (modeling relationships as Classes) to support N-ary participants and characteristics.
%     \item Participant Paths find a powerful equivalent in OWL Property Chains, enabling deep graph traversal inference.
%     \item Queries map isomorphically to SPARQL, and Views align with SHACL Shapes.
% \end{itemize}
% By adopting these mappings, systems engineers can document their domains in UDDL to satisfy rigorous documentation requirements~\cite{OpenGroup2023UDDL}, and then project these models into OWL to gain the immense benefits of the Semantic Web: machine reasoning, global interoperability, and automated data integration. The result is a unified approach that eliminates ambiguity not just for humans reading the spec, but for the machines executing it.
