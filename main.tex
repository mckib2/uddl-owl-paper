...
The engineering of complex, software-intensive systems is undergoing a fundamental shift toward the Modular Open Systems Approach (MOSA), where machine-interpretable data is a prerequisite for system integration.  Despite the strategic initiatives, a pervasive challenge remains: the inability to formally bind semantics to data syntax.  While the Universal Domain Description Language (UDDL)\textregistered{} provides a robust framework for documenting data meaning, it lacks the native reasoning capabilities to verify if those definitions are physically or operationally meaningful within a system architecture.
...
Individuals in the generated ontology with properties matching expecting semantics were queried and found using converted SPARQL queries.  Because we identify individuals based on semantics, we can make inferences about any individual Observations in the ontology to test if they align with the data an interface expects.
