...
The engineering of complex, software-intensive systems is undergoing a fundamental shift toward Digital Engineering and Intelligent Digital Twins, where machine-interpretable data is a prerequisite for system integration.  Despite strategic initiatives like the Modular Open Systems Approach (MOSA), a pervasive challenge remains: the inability to formally bind semantics to data syntax.  While the Universal Domain Description Language (UDDL)\textregistered{} provides a robust framework for documenting data meaning, it lacks the native reasoning capabilities to verify if those definitions are physically or operationally meaningful.
...
Traditional document-based approaches fail to capture data semantics due to their inability to precisely bind meaning to structure~\parencite{price2013semantic, Friedenthal2014}, often resulting in heterogeneous documentation across disparate systems like avionics and the  Industrial Internet of Things (IIoT)~\parencite{mzili2025interoperability, noura2019interoperability}.  While UDDL was developed specifically to eliminate this ambiguity by creating formal conceptual building blocks for data interfaces, it remains under-studied in systems and interoperability literature.
...
\begingroup
    ...
    \begin{tabularx}{\textwidth}{@{} >{\sffamily\raggedright\arraybackslash}p{0.35\textwidth} >{\sffamily\raggedright\arraybackslash}X @{}}
        \rowcolor{tableheader}
        \headingfont\bfseries UDDL Model Summary Statistics & \headingfont\bfseries Model Examples \\
        \addlinespace[4pt]
...
The reverse mapping (OWL to UDDL) is not considered in this paper.  Most UDDL concepts have unambiguous mappings from UDDL to OWL, however, OWL allows for more constructs and relations than are easily mapped back to UDDL.  Future work should consider what restrictions on ontology construction must be enforced to guarantee two-way mappings without loss of information.
...
Practitioners using this methodology can move beyond ``human-in-the-loop'' interface reviews by integrating OWL-based reasoning into the Digital Engineering pipeline.  Organizations can programmatically identify semantically incoherent interface definitions (e.g., mismatched sensor-to-sink logic) during the design phase.  This ``shift-left'' approach transforms the manual review bottleneck into a scalable, automated governance process.

The bridge between UDDL and OWL provides a mechanism to enforce a Technical Source of Truth across disparate system components.  While UDDL maintains the structure required for FACE or SOSA alignment, the OWL mapping provides formal rigor that provides evidence that data exchanged between a system's subsystems is logically consistent and operationally valid.

For programs adhering to Modular Open Systems Approaches (MOSA), the proposed methodology enables the creation of modular, reusable ``Semantic Libraries''.  By mapping domain-specific UDDL models to distinct ontologies, systems engineers can verify the interoperability of new components against existing reference architectures with minimal manual effort, significantly reducing the risk and cost associated with long-term system evolution and technology insertion.
