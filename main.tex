...
\begin{table*}[t]
  ...
  \end{tabularx}
  \caption{Core UDDL Conceptual Meta-Model Elements.  CDM, Domain, Basis Entity, and Observable examples are referenced from \cite{sdm_v3_1_15}.} \label{tab:cdm_elements}
\end{table*}
...
\begingroup
    ...
    \begin{tabularx}{\textwidth}{@{} >{\sffamily\raggedright\arraybackslash}p{0.35\textwidth} >{\sffamily\raggedright\arraybackslash}X @{}}
        ...
        \textit{Participants}: charger: SolarPanel, charged: Battery \par\vspace{6pt}

        \textbf{Observation Pattern}: TemperatureSensor\_Observe\_LouverFrame \newline
        \textit{Compositions}: Identifier \newline
        \textit{Participants}: observer: TemperatureSensor, observed: Frame\_PartOf\_Louver $\to$ part \par\vspace{6pt}

        \textbf{Assembly Pattern}: Blade\_PartOf\_Louver \newline
        \textit{Compositions}: Identifier, HealthState \newline
        \textit{Participants}: assembly: Louver, part: Blade
    \end{tabularx}
