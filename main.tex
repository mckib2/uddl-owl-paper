...
\begin{table*}[t]
    ...
    \begin{tabular}{|p{0.25\linewidth}|p{0.35\linewidth}|p{0.35\linewidth}|}
        \hline
        \textbf{Description} & \textbf{UDDL Query} & \textbf{SPARQL} \\
        \hline
        Retrieve Satellite Position & \texttt{SELECT Position FROM Satellite} & \texttt{SELECT ?pos WHERE \{ ?s a :Satellite . ?s :hasPosition ?pos \}} \\
        \hline
        Retrieve Sensor Temperature & \texttt{SELECT Temperature FROM Sensor JOIN Component ON Sensor.partOf = Component} & \texttt{SELECT ?temp WHERE \{ ?s a :Sensor . ?s :partOf ?c . ?c a :Component . ?s :measures ?temp \}} \\
        \hline
        Invalid Mass Measurement & \texttt{SELECT Mass FROM TempSensor} & \texttt{SELECT ?mass WHERE \{ ?s a :TempSensor . ?s :measures ?mass . ?mass a :Mass \}} \\
        \hline
    \end{tabular}
