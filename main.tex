...
\begin{table*}[t]
  ...
  \begin{tabularx}{\textwidth}{@{}A{0.25\textwidth}A{0.35\textwidth}>{\raggedright\arraybackslash}X@{}}
      ...
      \headingfont\bfseries Model Level & \headingfont\bfseries Description & \headingfont\bfseries Semantic Focus \\
      \addlinespace[4pt]
      ...
      \addlinespace[4pt]
      Platform Data Model (PDM) & Refines elements with Platform Data Types. & Implementation.  Additional characterization of logical measurements to include bit-level precision and language bindings in terms of Interface Definition Language (IDL).
  \end{tabularx}
  \caption{UDDL Multi-level Modeling Approach.  This table illustrates the hierarchical separation of concerns within the Universal Domain Description Language (UDDL).  It defines a progression from the Conceptual Data Model (CDM), where core domain semantics and relationships are established, to the Logical Data Model (LDM) for quantification (units and reference frames), and finally to the Platform Data Model (PDM) for IDL type association and language bindings.  This paper focuses on the CDM level as it contains the complete semantic description required for formal mapping to OWL and automated verification.}
...
\begin{table*}[t]
  \centering
  \renewcommand{\arraystretch}{1.5}
  \begin{tabularx}{\textwidth}{@{}A{0.20\textwidth}A{0.40\textwidth}>{\raggedright\arraybackslash}X@{}}
      \rowcolor{tableheader}
      \headingfont\bfseries Element Name & \headingfont\bfseries Construction & \headingfont\bfseries Example \\
      \addlinespace[4pt]
      Conceptual Data Model & Packages conceptual model elements in a single collection. & \texttt{Observables} (a collection of all Observable data model elements). \\
      \addlinespace[4pt]
      Domain & A collection of Basis Entities. & \texttt{Sensing}; includes the \texttt{Object} Basis Entity. \\
      \addlinespace[4pt]
      Basis Entity & Unique axiomatic domain concept. & \texttt{Place} \\
      \addlinespace[4pt]
      Observable & Unique trait which can be observed but not further characterized. & \texttt{Position} \\
      \addlinespace[4pt]
