...
The initial phase traverses the UDDL Query specification Abstract Syntax Tree (AST) to resolve all navigational steps into flattened Participant Paths (the same form used for Association participants).  During this traversal, an Alias Map ($ \mathcal{M} $) is constructed, where each key represents a unique entity alias $ a $ and the value is a set of paths $ \mathcal{P} $ reaching that entity.
